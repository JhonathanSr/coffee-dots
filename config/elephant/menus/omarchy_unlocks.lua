--
-- Dynamic  Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the -shipped Plymouth via
-- -plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs -plymouth-set-by-theme
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config//themes"
  local _path = os.getenv("_PATH") or ""
  local default_themes_dir = _path .. "/themes"
  local default_preview = _path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "-launch-floating-terminal-with-presentation "
                .. shell_escape("-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
